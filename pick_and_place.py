...
def listen_to_rostopic():
    try:
        ...
        while i <= NUMBER_OF_CUBES:     # For every cube
            ...
            cube_coords = [float(output_list[2]), float(output_list[3]), float(output_list[4]), output_list[5]]  # Save the data
...
class NumberSubmitter(QWidget):
    ...
    def initUI(self):
        ...
        self.checkboxes = []
        for cube in self.cube_positions:
            checkbox = QCheckBox("Cube {}".format(i), self)
            checkbox.setStyleSheet("color : {}".format(cube[3]))
